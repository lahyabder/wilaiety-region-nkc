...
const LicenseImageUpload = ({ 
  licenseId, 
  currentImageUrl, 
  onImageUploaded, 
  isEditing 
}: LicenseImageUploadProps) => {
  const { t } = useLanguage();
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileName = `licenses/${licenseId}/${Date.now()}.${fileExt}`;
      ...
      toast.error(t("Échec du téléchargement de l'image", "فشل في رفع الصورة") + ": " + (error.message || t("Erreur inconnue", "خطأ غير معروف")));
    } finally {
      setUploading(false);
    }
  };
        ...
        >
          {uploading ? (
            <div className="text-center">
              ...
              <span className="text-sm text-muted-foreground">{t("Téléchargement en cours...", "جاري الرفع...")}</span>
            </div>
          ) : isEditing ? (
            <div className="text-center">
              <Image className="w-8 h-8 text-muted-foreground mx-auto mb-2" />
              <span className="text-sm text-muted-foreground">{t("Cliquez pour télécharger une image de licence", "انقر لرفع صورة الترخيص")}</span>
